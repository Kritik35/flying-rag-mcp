...
class TableQueryTests(unittest.TestCase):
    def test_resolve_files_respects_dataset_filter_without_source_like(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            save_parent_chunk(db_path, "p-norm", r"C:\Docs\Norm\spec.xlsx", "дорегулирование 10 Па")
            save_parent_chunk(db_path, "p-proj", r"C:\Docs\Project\ov2.xlsx", "дорегулирование 20 Па")

            original_meta_db = table_query._meta_db
            try:
                table_query._meta_db = lambda: db_path
                files = table_query._resolve_files("дорегулирование", None, "project", 10)
            finally:
    ...
    def test_resolve_files_respects_dataset_filter_with_source_like(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            save_parent_chunk(db_path, "p-norm", r"C:\Docs\Norm\OV2\spec.xlsx", "дорегулирование 10 Па")
            save_parent_chunk(db_path, "p-proj", r"C:\Docs\Project\OV2\ov2.xlsx", "дорегулирование 20 Па")

            original_meta_db = table_query._meta_db
            try:
                table_query._meta_db = lambda: db_path
                files = table_query._resolve_files("дорегулирование", "OV2", "project", 10)
            finally:
                table_query._meta_db = original_meta_db
    ...
    def test_resolve_files_prioritizes_subject_hits_with_source_like(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            save_parent_chunk(db_path, "p2", relevant, "дорегулирование 20 Па")

            original_meta_db = table_query._meta_db
            try:
                table_query._meta_db = lambda: db_path
                files = table_query._resolve_files("дорегулирование", "OV2", "project", 1)
            finally:
                table_query._meta_db = original_meta_db
